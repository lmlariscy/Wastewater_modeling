...
## A) WBE Wrangling - long data edition
...
### 2. Average qPCR technical & biological replicates
...
### 7. Calculate WBE 7-DMA (Don't need this)
...
### 2. Calculate Case 7-DMA (Don't need this)
...
```{r}
class_data <- covid_wbe_7dma_weekly %>% mutate(case_prop_pos = prop_pos*100,
                                               ) %>% 
  select(c(week,avg_vl,avg_pos_rate,cases.symptom.onset,cases.reported,
           pcr_tests,pcr_pos,case_prop_pos))

write_csv(class_data,here("data/processed_data/STAT8040_data.csv"))
saveRDS(class_data,here("data/processed_data/STAT8040_data.rds"))
```

## D) WBE Wrangling - wide data edition

### PIVOT! (widen avg viral load data)

```{r}
wbe_vl_daily_wide <- wbe_avg %>% select(c(date,avg_viral_load,facility,target)) %>% 
...
### PIVOT! (widen qpcr pos data)

```{r}
wbe_npos_daily_wide <- qpcr_na %>% mutate(facility = as.factor(facility), #code each facility as a factor
        ...
        ungroup() %>% 
        select(!n_miss) %>% 
        pivot_wider(names_from = c(facility,target), values_from = c(n, n_pos))
                                   
```

### Combine viral load and npos data

```{r}
wbe_weekly_wide <- left_join(wbe_npos_daily_wide,wbe_vl_daily_wide, by = "date") %>% 
  group_by(week) %>% 
  summarise_at(vars(2:19), mean, ra.rm = F)
```

### Combine WBE wide data with COVID daily data, summarize by week

```{r}
covid_weekly <- covid_daily %>% 
  select(!c(cases.symptom.onset,cases.reported,pcr_pos_7dma,pcr_pos_14dma,prop_pos,date)) %>% 
  group_by(week) %>% 
  summarise_at(vars(1:2),sum,na.rm = F) %>% 
  ungroup()

wbe_covid_weekly_wide <- left_join(wbe_weekly_wide, covid_weekly, by = "week") %>% drop_na(week)

saveRDS(wbe_covid_weekly_wide,here("data/processed_data/wbe_covid_weekly_wide.rds"))
