...
-   Transforms NAs to LOD for each assay (and sensitivity testing w/ NAs set at 40 and 42)
...
#### Transform Ct NAs, then calculate copies/uL

##### Using LOD

```{r}
#Calculate half LOQ for each assay, assign to rows greater than LOQ
...
qpcr_plant_all <- merge(qpcr_all, plant_v2, 
  ...
  mutate(facility = as.factor(facility), #code each facility as a factor
        ...
        #transform copies per uL of reaction to copies per liter
         drop_na(target) %>% 
         select(!notes)
...
```{r}
#Merge and mutate
qpcr_plant_all2 <- merge(qpcr_all_2, plant_v2, 
                        by = c("collection_num", "facility", "date"), all = T) %>% 
  mutate(facility = as.factor(facility), #code each facility as a factor
         facility = recode(facility, NO = "A", MI = "B", CC = "C"), 
         #de-identify treatment facility
         facility = ordered(facility, levels = c("A", "B", "C")), 
...
#Save to processed data folder
saveRDS(qpcr_plant_all2, here("data/processed_data/qpcr_plant_all_NA40.rds"))
```
...
```{r}
#Merge and mutate
qpcr_plant_all3 <- merge(qpcr_all_3, plant_v2, 
                        by = c("collection_num", "facility", "date"), all = T) %>% 
  mutate(facility = as.factor(facility), #code each facility as a factor
         facility = recode(facility, NO = "A", MI = "B", CC = "C"), 
         #de-identify treatment facility
...
#Save to processed data folder
saveRDS(qpcr_plant_all3, here("data/processed_data/qpcr_plant_all_NA42.rds"))
```

### 3. Quick look at WBE data
