...
set.seed(13)
bootstrap_vl_n6 <- bootstraps(data_n6_train, times = 50)

#run bootstrapping and save predictions
bootstrap_vl_n6_results <- fit_resamples(vl_workflow_n6, 
        resamples = bootstrap_vl_n6,
        control = control_resamples(save_pred = TRUE))

#collect predictions and mutate back to original unit (log10 transformed prior to running model)
predictions_vl_n6 <- collect_predictions(bootstrap_vl_n6_results, summarize = FALSE) %>% 
...
predictions_vl_n6 %>% ggplot(aes(actual, prediction)) +
  geom_point() +
...
predictions_vl_n6_sum <- predictions_vl_n6 %>%
  group_by(.row) %>%
  summarize(
    actual = mean(actual),
...
vl_n6_cor <- cor(predictions_vl_n6_sum$actual,
         predictions_vl_n6_sum$prediction_avg,
         method = "spearman")

#plot timeseries
predictions_vl_n6_sum %>% 
  ggplot(aes(.row)) +
...
#create bootstraps for predictions
set.seed(13)
bootstrap_vl_n6_full <- bootstraps(data_n3, times = 50)

#run bootstrapping and save predictions
bootstrap_vl_n6_results_full <- fit_resamples(vl_workflow_n6, 
        resamples = bootstrap_vl_n6_full,
        control = control_resamples(save_pred = TRUE))

#collect predictions and mutate back to original unit (log10 transformed prior to running model)
predictions_vl_n6_full <- collect_predictions(bootstrap_vl_n6_results_full, summarize = FALSE) %>% 
mutate(prediction = 10^.pred,
       actual = 10^log10_cases)

#calculate 95% confidence interval for predictions
predictions_vl_n6_sum_full <- predictions_vl_n6_full %>%
  group_by(.row) %>%
  summarize(
    actual = mean(actual),
    prediction_avg = mean(prediction),
