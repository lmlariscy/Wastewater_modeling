...
#all metrics data
lm_metrics <- readRDS(
  here("data/processed_data/linear_reg/metrics_compare.rds")) %>% mutate(model = "linear")
rf_metrics <- readRDS(
  here("data/processed_data/rand_forest/metrics_compare.rds")) %>% mutate(model = "random_forest")
...
metric_summary <- rbind(lm_metrics,rf_metrics) %>% 
  select(model,feature,.config,.metric,mean,std_err) %>% 
  mutate(.config = str_sub(.config, -1, -1)) %>% 
  rename(bio_rep = .config,
         SE = std_err) %>% 
  pivot_wider(names_from = .metric,
              values_from = c(mean,SE)) %>% 
  select(model, feature, bio_rep, mean_rsq, SE_rsq, mean_rmse, SE_rmse)
...
```{r}
#All RF
rf_lm_rmse %>% filter(feature == "combined" & model == "rand_forest")

#All LM
rf_lm_rmse %>% filter(feature == "combined" & model == "linear_reg")

#DF RF
rf_lm_rmse %>% filter(feature == "df" & model == "rand_forest")

#DF LM
rf_lm_rmse %>% filter(feature == "df" & model == "linear_reg")

#VL RF
rf_lm_rmse %>% filter(feature == "vl" & model == "rand_forest")

#VL LM
rf_lm_rmse %>% filter(feature == "vl" & model == "linear_reg")
```
...
```{r}
#All RF
rf_lm_rsq %>% filter(feature == "combined" & model == "rand_forest")

#All LM
rf_lm_rsq %>% filter(feature == "combined" & model == "linear_reg")

#DF RF
rf_lm_rsq %>% filter(feature == "df" & model == "rand_forest")

#DF LM
rf_lm_rsq %>% filter(feature == "df" & model == "linear_reg")

#VL RF
rf_lm_rsq %>% filter(feature == "vl" & model == "rand_forest")

#VL LM
rf_lm_rsq %>% filter(feature == "vl" & model == "linear_reg")
```
