...
#DF 
start_2022_rsq_df <- df_n3_test_predictions %>% 
  head(17) %>% 
  rsq(truth = actual, estimate = prediction)

start_2022_rsq_df

end_2022_rsq_df <- df_n3_test_predictions %>% 
  tail(33) %>% 
  rsq(truth = actual, estimate = prediction)
...
#ALL
start_2022_rsq_all <- all_n6_test_predictions %>% 
  head(17) %>% 
  rsq(truth = actual, estimate = prediction)

start_2022_rsq_all

end_2022_rsq_all <- all_n6_test_predictions %>% 
  tail(33) %>% 
  rsq(truth = actual, estimate = prediction)
...
rmse_compare <- metrics_compare %>% 
  ...
  geom_hline(yintercept = 0.45, alpha = 0) +
  geom_hline(yintercept = 0.2, alpha = 0)
...
saveRDS(mean_rsq,
        here("data/processed_data/linear_reg/rsq_mean_compare.rds"))
```
...
all_n6_train_fig <- all_n6_train_predictions %>% 
  ...
  #xlab("Week") +
  geom_hline(yintercept = 3750, alpha = 0) +
  ggthemes::theme_clean() +
...
```{r}
all_n6_test1_rsq_round <- start_2022_rsq_all$.estimate %>% 
  round(digits = 3)
all_n6_test2_rsq_round <- end_2022_rsq_all$.estimate %>% 
  round(digits = 3)
...
all_n6_test_fig <- all_n6_test_predictions %>% 
  ...
  geom_line(aes(y=actual), color = "#003e99") +
  geom_point(aes(y=actual), color = "#003e99", shape = 16) +
  annotate("text", label=all_n6_test1_rsq_round, x=19095, y=6000, size=4) +
  annotate("text", label=all_n6_test2_rsq_round, x=19330, y=6000, size=4) +
  annotate("text", label="Predicted", x=19045, y=5650, color="#FB6A4A", size=4) +
  annotate("text", label="Observed", x=19045, y=6000, color="#003e99", size=4) +
  ylab("Weekly reported cases") +
  #xlab("Week") +
...
```{r}
df_n3_test1_rsq_round <- start_2022_rsq_df$.estimate %>% 
  round(digits = 3)
df_n3_test2_rsq_round <- end_2022_rsq_df$.estimate %>% 
  round(digits = 3)
...
df_n3_test_fig <- df_n3_test_predictions %>% 
  ...
  geom_line(aes(y=actual), color = "#003e99") +
  geom_point(aes(y=actual), color = "#003e99", shape = 16) +
  annotate("text", label=df_n3_test1_rsq_round, x=19095, y=6000, size=4) +
  annotate("text", label=df_n3_test2_rsq_round, x=19330, y=6000, size=4) +
  annotate("text", label="Predicted", x=19045, y=5650, color="#FB6A4A", size=4) +
  annotate("text", label="Observed", x=19045, y=6000, color="#003e99", size=4) +
  #ylab("Weekly reported cases") +
  #xlab("Week") +
